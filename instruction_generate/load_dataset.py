...
def time_add(data, dataset_name, week_start, month_day_start, month_start, interval=5, weekday_only=False, year_list=None, holiday_list=None, month_list_spe=None, day_start=0, hour_of_day=24):
    ...
    elif dataset_name == 'SD2017':
        type_data = type_data + 7
    elif dataset_name == 'SD2021':
        type_data = type_data + 7
    elif dataset_name == 'SZ2022':
        type_data = type_data + 8
    elif dataset_name == 'SH2015':
        type_data = type_data + 9
    elif dataset_name == 'pems08' or dataset_name == 'pems03' or dataset_name == 'pems04':
        type_data = type_data + 10
    elif dataset_name == 'urbanev':
        type_data = type_data + 11
    else:    
        raise ValueError
...
def load_st_dataset(dataset, args):
    ...
    if dataset == 'NYCmulti':
        ...
        data_tflow = data_tflow[:80, (0) * ((24 * 60) // interval):(7) * ((24 * 60) // interval), :]
        data = data_tflow
    elif dataset == 'CA_2017':
        data_taxi_path = os.path.join('data/st_data/ca_his_raw_2017.h5')


        with h5py.File(data_taxi_path, 'r') as file:
            data_taxi = file['t/block0_values'][:].T
            data_taxi = np.expand_dims(data_taxi, axis=-1)
        month_start = 1
        week_start = 7
        month_day_start = 1
        holiday_list = None
        ...
        args.month=month
        day_data, week_data, month_day_data, month_data, year_data, type_data = time_add(
            data_taxi, 'SD2017', week_start, month_day_start, month_start, interval=interval, weekday_only=False, year_list = year_list,
            holiday_list=holiday_list)
        data_taxi = np.concatenate([data_taxi, day_data, week_data, month_day_data, month_data, year_data, type_data], axis=-1)
        ...
        # np.savez('NYC_taxi_mask4', data=mask4)
        data = data_taxi
    elif dataset == 'SD_2017':
        data_taxi_path = os.path.join('./data/st_data/sd/sd_his_2017.h5')

        with h5py.File(data_taxi_path, 'r') as file:
            data_taxi = file['t/block0_values'][:]
            df_taxi = pd.DataFrame(data_taxi)
            df = df_taxi.astype(float)
            print(f"NaN 的数量（插值前）: {df_taxi.isna().sum().sum()}")
            columns_to_drop = [col for col in df.columns if has_consecutive_nans(df[col])]
            df_cleaned = df.drop(columns=columns_to_drop)
            df_interpolated = df_cleaned.interpolate(method='linear', axis=0, limit_direction='both')
            print(f"NaN 的数量（插值后）: {df_interpolated.isna().sum().sum()}")
            data_taxi = df_interpolated.to_numpy().T
            data_taxi = np.expand_dims(data_taxi, axis=-1)
            data_taxi = np.tile(data_taxi, (1, 1, 2))
        month_start = 1
        week_start = 7
        month_day_start = 1
        holiday_list = None
        ...
        if args.for_zeroshot:
            data_taxi = data_taxi[80:160, (-5) * (24 * 60 // 5):, :]
        elif args.for_supervised:
            data_taxi = data_taxi[:, (-10) * (24 * 60 // 5):, :]
        elif args.for_ablation:
            data_taxi = data_taxi[80:160, (366) * (24 * 60 // 5):(366 + (31 + 28 + 31)) * (24 * 60 // 5), :]
        elif args.for_test:
            data_taxi = data_taxi[:, (-36) * (24 * 60 // 5):(-35) * (24 * 60 // 5), :]
        else:
            raise ValueError
        ...
        # np.savez('NYC_taxi_mask4', data=mask4)
        data = data_taxi
    elif dataset == 'SD_2021':
        data_taxi_path = os.path.join('./data/st_data/sd/sd_his_2021.h5')
        ...
        args.month=month
        day_data, week_data, month_day_data, month_data, year_data, type_data = time_add(
            data_taxi, 'SD2021', week_start, month_day_start, month_start, interval=interval, weekday_only=False, year_list = year_list,
            holiday_list=holiday_list)
        ...
        if args.for_zeroshot:
            ...
            # data_taxi = data_taxi[:, (-180) * (24 * 60 // 5):, :]
            data_taxi = data_taxi[:, (-9) * (24 * 60 // 5):, :]
            # data_taxi = data_taxi[:, (-4) * (2*60 // 5)-1:, :]
        elif args.for_ablation:
            data_taxi = data_taxi[80:160, (366) * (24 * 60 // 5):(366 + (31 + 28 + 31)) * (24 * 60 // 5), :]
        elif args.for_test:
            # data_taxi = data_taxi[:, (-36) * (24 * 60 // 5):(-35) * (24 * 60 // 5), :]
            data_taxi = data_taxi[:, (-36-7) * (24 * 60 // 5):(-35) * (24 * 60 // 5), :]
            # data_taxi = data_taxi[:, (-4) * (24 * 60 // 5):(-4) * (24 * 60 // 5)+48, :]
        else:
            raise ValueError
        ...
        data_taxi_path = os.path.join('./data/st_data/pems08/pems08.npz')
        data_taxi = np.load(data_taxi_path)['data']
        ...
        args.month=month
        day_data, week_data, month_day_data, month_data, year_data, type_data = time_add(
            data_taxi, 'pems08', week_start, month_day_start, month_start, interval=interval, weekday_only=False, year_list = year_list,
            holiday_list=holiday_list)
        ...
        if args.for_zeroshot:
            ...
            # data_taxi = data_taxi[:, (-180) * (24 * 60 // 5):, :]
            # data_taxi = data_taxi[:, (-11) * (24 * 60 // 5):, :]
            data_taxi = data_taxi[:, (-21-7) * (24 * 60 // 5):(-17) * (24 * 60 // 5), :]
            # data_taxi = data_taxi[:, (-18) * (24 * 60 // 5):(-17) * (24 * 60 // 5), :]
            # data_taxi = data_taxi[:, (-1) * (2*60 // 5)-1:, :]
        elif args.for_ablation:
            data_taxi = data_taxi[80:160, (366) * (24 * 60 // 5):(366 + (31 + 28 + 31)) * (24 * 60 // 5), :]
        elif args.for_test:
            # data_taxi = data_taxi[:, (-21-7) * (24 * 60 // 5):(-20) * (24 * 60 // 5), :]
            data_taxi = data_taxi[:, (-8) * (24 * 60 // 5):, :]
            # data_taxi = data_taxi[:, (-4) * (24 * 60 // 5):(-4) * (24 * 60 // 5)+48, :]
        else:
            raise ValueError
        data = data_taxi
    elif dataset == 'pems03':
        data_taxi_path = os.path.join('./data/st_data/pems03/pems03_clip.npz')
        data_taxi = np.load(data_taxi_path)['data']
        data_taxi = np.transpose(data_taxi, (1, 0, 2))
        data_taxi = np.concatenate([data_taxi, data_taxi], axis=-1)
        month_start = 9
        ...
        args.month=month
        day_data, week_data, month_day_data, month_data, year_data, type_data = time_add(
            data_taxi, 'pems03', week_start, month_day_start, month_start, interval=interval, weekday_only=False, year_list = year_list,
            holiday_list=holiday_list)
        ...
        args.month=month
        day_data, week_data, month_day_data, month_data, year_data, type_data = time_add(
            data_taxi, 'pems04', week_start, month_day_start, month_start, interval=interval, weekday_only=False, year_list = year_list,
            holiday_list=holiday_list)
        ...
        data_taxi_path = os.path.join('./data/st_data/urbanev/urbanev-charge.npz')
        data_taxi = np.load(data_taxi_path)['data']
        data_taxi = np.transpose(data_taxi, (1, 0, 2))
        data_taxi = np.concatenate([data_taxi, data_taxi], axis=-1)
        month_start = 9
        ...
        args.month=month
        day_data, week_data, month_day_data, month_data, year_data, type_data = time_add(
            data_taxi, 'urbanev', week_start, month_day_start, month_start, interval=interval, weekday_only=False, year_list = year_list,
            holiday_list=holiday_list)
        ...
        if args.for_zeroshot:
            ...
            # data_taxi = data_taxi[:, (-180) * (24 * 60 // 5):, :]
            # data_taxi = data_taxi[:, (-11) * (24 * 60 // 5):, :]
            data_taxi = data_taxi[:, (-50) * (24):(-17) * (24), :]
            # data_taxi = data_taxi[:, (-18) * (24 * 60 // 5):(-17) * (24 * 60 // 5), :]
            # data_taxi = data_taxi[:, (-1) * (2*60 // 5)-1:, :]
        elif args.for_ablation:
            data_taxi = data_taxi[80:160, (366) * (24 * 60 // 5):(366 + (31 + 28 + 31)) * (24 * 60 // 5), :]
            ...
            # data_taxi = data_taxi[:, (-21-7) * (24 * 60 // 5):(-20) * (24 * 60 // 5), :]
            data_taxi = data_taxi[:, (-10) * (24):, :]
            # data_taxi = data_taxi[:, (-4) * (24 * 60 // 5):(-4) * (24 * 60 // 5)+48, :]
        else:
            raise ValueError
        data = data_taxi
    elif dataset == 'SZ_2022':
        data_taxi_path = os.path.join('./data/st_data/shenzhen/sz-charge.npz')
        data_taxi = np.load(data_taxi_path)['data']
        data_taxi = np.transpose(data_taxi, (1, 0, 2))
        data_taxi = np.concatenate([data_taxi, data_taxi], axis=-1)
        month_start = 7
        ...
        args.month=month
        day_data, week_data, month_day_data, month_data, year_data, type_data = time_add(
            data_taxi, 'SZ2022', week_start, month_day_start, month_start, interval=interval, weekday_only=False, year_list = year_list,
            holiday_list=holiday_list)
        ...
        # np.savez('NYC_taxi_mask4', data=mask4)
        data = data_taxi
    elif dataset == 'SH_2015':
        data_taxi_path = os.path.join('./data/st_data/shanghai/2015/his.npz')
        data_taxi = np.load(data_taxi_path)['demand_count']
        data_taxi1 = np.load(data_taxi_path)['empty_count']
        data_taxi2 = np.load(data_taxi_path)['waiting_count']
        data_taxi = np.expand_dims(data_taxi, axis=-1) 
        data_taxi = np.transpose(data_taxi, (1, 0, 2))
        data_taxi1 = np.expand_dims(data_taxi1, axis=-1) 
        data_taxi1 = np.transpose(data_taxi1, (1, 0, 2))
        data_taxi2 = np.expand_dims(data_taxi2, axis=-1) 
        data_taxi2 = np.transpose(data_taxi2, (1, 0, 2))
        data_taxi = np.concatenate([data_taxi, data_taxi1,data_taxi2], axis=-1)
        month_start = 4
        week_start = 1 
        month_day_start = 13
        holiday_list = None
        ...
        args.month=month
        day_data, week_data, month_day_data, month_data, year_data, type_data = time_add(
            data_taxi, 'SH2015', week_start, month_day_start, month_start, interval=interval, weekday_only=False, year_list = year_list,
            holiday_list=holiday_list)
        
        data_taxi = np.concatenate([data_taxi, day_data, week_data, month_day_data, month_data, year_data, type_data], axis=-1)
        file_path = 'third_task_preprocess/empty_transfer_prob.npy'
        real_prob = np.load(file_path)
        real_prob =np.transpose(real_prob, (1, 0, 2))
        if args.for_zeroshot:
            data_taxi = data_taxi[80:160, (-5) * (24 * 60 // 5):, :]
        elif args.for_supervised:
            # data_taxi = data_taxi[:, (-180) * (24 * 60 // 5):, :]
            data_taxi = data_taxi[:, :(-3) * (24 * 60 // 5), :]
            real_prob = real_prob[:, :(-3) * (24 * 60 // 5), :]
            # data_taxi = data_taxi[:, (-1) * (2*60 // 5)-1:, :]
        elif args.for_ablation:
            data_taxi = data_taxi[80:160, (366) * (24 * 60 // 5):(366 + (31 + 28 + 31)) * (24 * 60 // 5), :]
        elif args.for_test:
            data_taxi = data_taxi[:, (-144):, :]
            real_prob = real_prob[:, (-144):, :]
            # data_taxi = data_taxi[:, (-4) * (24 * 60 // 5):(-4) * (24 * 60 // 5)+48, :]
        else:
            raise ValueError
    ...
    if dataset == 'SH_2015':
        ...
        print('Load %s Dataset shaped: ' % dataset, data.shape, data[..., 2:3].max(), data[..., 2:3].min(),
            data[..., 2:3].mean(), np.median(data[..., 2:3]), data.dtype)
        print('time: ', data[..., 3:4].max(), data[..., 3:4].min())
        print('week: ', data[..., 4:5].max(), data[..., 4:5].min())
        print('day: ', data[..., 5:6].max(), data[..., 5:6].min())
        print('month: ', data[..., 6:7].max(), data[..., 6:7].min())
        print('year: ', data[..., 7:8].max(), data[..., 7:8].min())
        print('type: ', data[..., 8:9].max(), data[..., 8:9].min())
        return data,real_prob
    else: 
        print('Load %s Dataset shaped: ' % dataset, data.shape, data[..., 0:1].max(), data[..., 0:1].min(),
          data[..., 0:1].mean(), np.median(data[..., 0:1]), data.dtype)
        ...
        return data
def has_consecutive_nans(series, threshold=20):
    is_nan = series.isna()
    consecutive_nan_lengths = (is_nan != is_nan.shift()).cumsum()[is_nan].value_counts()
    return consecutive_nan_lengths.max() > threshold if not consecutive_nan_lengths.empty else False
